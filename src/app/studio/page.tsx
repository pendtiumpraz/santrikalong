        ...
        <div className="content">
          ...
          <section data-pane="earn" hidden>
            ...
            <div className="grid-2" style={{ gridTemplateColumns: "1.2fr 1fr" }}>
              ...
              <div className="card card-pad"><p style={{ fontWeight: 600, marginBottom: ".6rem" }}>Status payout</p>{payouts.length === 0 ? <p className="muted" style={{ fontSize: ".85rem" }}>Belum ada pengajuan payout.</p> : payouts.map((p) => (<p key={p.id} style={{ fontSize: ".85rem", marginBottom: ".4rem", display: "flex", alignItems: "center", gap: ".5rem" }}>{idr(p.netIdr)} · <span className={p.status === "PAID" ? "tag tag-success" : p.status === "REJECTED" ? "tag tag-muted" : "tag tag-warn"}>{p.status}</span><a href={`/bukti/payout/${p.id}`} target="_blank" rel="noopener" className="btn btn-ghost btn-sm" style={{ marginLeft: "auto" }}><svg className="ico ico-sm"><use href="#i-receipt" /></svg>Bukti</a></p>))}</div>
            </div>
