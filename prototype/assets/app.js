/* SantriKalong prototype — shared behaviors. Mengikuti docs/design/06 (theme engine). */
(function () {
  "use strict";

  // ---------- Icon sprite (inline; no emoji in UI) ----------
  var SPRITE = '<svg xmlns="http://www.w3.org/2000/svg" style="position:absolute;width:0;height:0" aria-hidden="true"><defs>' +
  ...
  var THEMES = ["pelita-malam", "kertas-subuh"];
  var MODES = ["light", "dark"];
  function set(el, k, v) { el.setAttribute("aria-pressed", el.getAttribute(k) === v); }

  function applyMode() {
    var m = document.documentElement.getAttribute("data-mode");
    var ic = document.querySelector("#sk-mode-ic use");
    if (ic) ic.setAttribute("href", m === "dark" ? "#i-sun" : "#i-moon");
    document.querySelectorAll("[data-set-mode]").forEach(function (b) { set(b, "data-set-mode", m); });
  }
  function setTheme(t) { document.documentElement.dataset.theme = t; try { localStorage.setItem("sk_theme", t); } catch (e) {} document.querySelectorAll("[data-set-theme]").forEach(function (b) { set(b, "data-set-theme", t); }); }
  function setMode(m) { document.documentElement.dataset.mode = m; try { localStorage.setItem("sk_mode", m); } catch (e) {} applyMode(); }
  function setText(s) { if (s === "large") document.documentElement.dataset.textsize = "large"; else document.documentElement.removeAttribute("data-textsize"); try { localStorage.setItem("sk_text", s); } catch (e) {} document.querySelectorAll("[data-set-text]").forEach(function (b) { set(b, "data-set-text", s); }); }
  ...
  function buildSwitcher() {
    ...
    d.innerHTML =
      ...
      '</div></div>' +
      '<button class="fab" aria-label="Tema & tampilan"><svg class="ico" style="width:22px;height:22px"><use href="#i-palette"/></svg></button>';
    document.body.appendChild(d);
    d.querySelector(".fab").addEventListener("click", function () { d.classList.toggle("open"); });
    d.querySelectorAll("[data-set-theme]").forEach(function (b) { b.onclick = function () { setTheme(b.dataset.setTheme); }; });
    d.querySelectorAll("[data-set-mode]").forEach(function (b) { b.onclick = function () { setMode(b.dataset.setMode); }; });
    d.querySelectorAll("[data-set-text]").forEach(function (b) { b.onclick = function () { setText(b.dataset.setText); }; });
  }

  function wire() {
    var s = document.createElement("div"); s.style.display = "none"; s.innerHTML = SPRITE; document.body.insertBefore(s, document.body.firstChild);
    buildSwitcher();
    var t = document.documentElement.getAttribute("data-theme") || "pelita-malam";
    var m = document.documentElement.getAttribute("data-mode") || "dark";
    setTheme(t); applyMode();
    document.querySelectorAll("[data-set-mode]").forEach(function (b) { set(b, "data-set-mode", m); });
    var ts = document.documentElement.getAttribute("data-textsize") === "large" ? "large" : "normal";
    document.querySelectorAll("[data-set-text]").forEach(function (b) { set(b, "data-set-text", ts); });

    var mt = document.getElementById("sk-mode-toggle");
    if (mt) mt.addEventListener("click", function () { setMode(document.documentElement.getAttribute("data-mode") === "dark" ? "light" : "dark"); });
    ...
    document.querySelectorAll("[data-tab]").forEach(function (btn) {
      btn.addEventListener("click", function () {
        var group = btn.closest("[data-tabgroup]") || document;
        group.querySelectorAll("[data-tab]").forEach(function (b) { b.setAttribute("aria-pressed", b === btn); });
        var sel = btn.getAttribute("data-tab");
        (group.querySelectorAll("[data-panel]")).forEach(function (p) { p.hidden = ("#" + p.getAttribute("data-panel")) !== sel && p.getAttribute("data-panel") !== sel.replace("#", ""); });
      });
    });
    ...
    document.querySelectorAll("[data-view]").forEach(function (btn) {
      btn.addEventListener("click", function () {
        ...
        var tt = document.getElementById("sk-page-title"); if (tt && btn.dataset.title) tt.textContent = btn.dataset.title;
        var sb = document.querySelector(".sidebar"); if (sb) sb.classList.remove("open");
      });
    });
    var sbt = document.getElementById("sk-sb-toggle");
    if (sbt) sbt.addEventListener("click", function () { document.querySelector(".sidebar").classList.toggle("open"); });

    document.querySelectorAll("[data-open]").forEach(function (b) { b.addEventListener("click", function () { var el = document.querySelector(b.getAttribute("data-open")); if (el) el.classList.add("open"); }); });
    document.querySelectorAll("[data-close]").forEach(function (b) { b.addEventListener("click", function () { var el = b.closest(".overlay,.drawer"); if (el) el.classList.remove("open"); }); });
